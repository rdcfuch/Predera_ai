...
class EasyAnimateController:
    def __init__(self):
        ...
        self.basedir = script_dir.replace("easyanimate/ui", "")
        ...
        self.inference_config = OmegaConf.load(
            os.path.join(self.config_dir, "easyanimate_video_magvit_motion_module_v2.yaml"))
    ...
    def update_edition(self, edition):
        ...
        if edition == "v1":
            ...
            return gr.update(), gr.update(value="none"), gr.update(visible=True), gr.update(visible=True), \
                ...
                gr.update(value=512, minimum=384, maximum=704, step=32), gr.update(value=80, minimum=40, maximum=80,
                                                                                   step=1)
        else:
            self.inference_config = OmegaConf.load(
                os.path.join(self.config_dir, "easyanimate_video_magvit_motion_module_v2.yaml"))
            return gr.update(), gr.update(value="none"), gr.update(visible=False), gr.update(visible=False), \
                ...
                gr.update(value=384, minimum=128, maximum=1280, step=16), gr.update(value=144, minimum=9, maximum=144,
                                                                                    step=9)
    ...
    def update_diffusion_transformer(self, diffusion_transformer_dropdown):
        ...
        self.text_encoder = T5EncoderModel.from_pretrained(diffusion_transformer_dropdown, subfolder="text_encoder",
        ...
        if is_image or length_slider == 1:
            ...
            if is_api:
                ...
                if gradio_version_is_above_4:
                    return gr.Image(value=save_sample_path, visible=True), gr.Video(value=None,
                                                                                    visible=False), "Success"
                else:
                    return gr.Image.update(value=save_sample_path, visible=True), gr.Video.update(value=None,
                                                                                                  visible=False), "Success"
        else:
            ...
            if is_api:
                ...
                if gradio_version_is_above_4:
                    return gr.Image(visible=False, value=None), gr.Video(value=save_sample_path,
                                                                         visible=True), "Success"
                else:
                    return gr.Image.update(visible=False, value=None), gr.Video.update(value=save_sample_path,
                                                                                       visible=True), "Success"
    ...
    with gr.Blocks(css=css) as demo:
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Row():
                ...
                diffusion_transformer_refresh_button.click(fn=refresh_diffusion_transformer, inputs=[],
                                                           outputs=[diffusion_transformer_dropdown])
            ...
            with gr.Row():
                ...
                lora_alpha_slider = gr.Slider(label="LoRA alpha (LoRA权重)", value=0.55, minimum=0, maximum=2,
                                              interactive=True)
        ...
        with gr.Column(variant="panel"):
            ...
            prompt_textbox = gr.Textbox(label="Prompt (正向提示词)", lines=2,
                                        value="This video shows the majestic beauty of a waterfall cascading down a cliff into a serene lake. The waterfall, with its powerful flow, is the central focus of the video. The surrounding landscape is lush and green, with trees and foliage adding to the natural beauty of the scene")
            negative_prompt_textbox = gr.Textbox(label="Negative prompt (负向提示词)", lines=2,
            ...
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        sampler_dropdown = gr.Dropdown(label="Sampling method (采样器种类)",
                                                       choices=list(scheduler_dict.keys()),
                                                       value=list(scheduler_dict.keys())[0])
                        sample_step_slider = gr.Slider(label="Sampling steps (生成步数)", value=50, minimum=10,
                                                       maximum=100, step=1)
                    ...
                    with gr.Row():
                        ...
                        length_slider = gr.Slider(label="Animation length (视频帧数)", value=144, minimum=9,
                                                  maximum=144, step=9)
                    ...
                    with gr.Row():
                        ...
                        seed_button.click(
                            fn=lambda: gr.Textbox(
                                value=random.randint(1, 1e8)) if gradio_version_is_above_4 else gr.Textbox.update(
                ...
                with gr.Column():
                    with gr.Row():
                        sampler_dropdown = gr.Dropdown(label="Sampling method", choices=list(scheduler_dict.keys()),
                                                       value=list(scheduler_dict.keys())[0])
                        sample_step_slider = gr.Slider(label="Sampling steps", value=50, minimum=10, maximum=100,
                                                       step=1)
                    ...
                    if edition == "v1":
                        ...
                        with gr.Column():
                            ...
                            with gr.Row():
                                ...
                                length_slider = gr.Slider(label="Animation length", value=72, minimum=9, maximum=81,
                                                          step=9)
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        sampler_dropdown = gr.Dropdown(label="Sampling method", choices=list(scheduler_dict.keys()),
                                                       value=list(scheduler_dict.keys())[0])
                        sample_step_slider = gr.Slider(label="Sampling steps", value=30, minimum=10, maximum=100,
                                                       step=1)
            ...
            generate_button.click(
                ...
                inputs=[
                    ...
                    negative_prompt_textbox,
                    sampler_dropdown,
                    sample_step_slider,"""Modified from https://github.com/guoyww/AnimateDiff/blob/main/app.py
"""
import gc
import json
import os
